...
We will use a boosted logit model with leave-one-out cross-validation (LOOCV). A boosted logit model is a type of ensemble method that combines multiple logistic regression models to improve predictive performance. Instead of fitting just one model, boosting fits a series of models sequentially, where each new model focuses on correcting the errors made by the previous ones. This can lead to a powerful final model that captures complex patterns in the data.
...
system.time(logit_boost <- train(My.Rating_bin9_cat ~ ., data = df_cv, method = "LogitBoost",
...
#predict
df_unseen$pred <- predict(logit_boost, newdata = df_unseen)
